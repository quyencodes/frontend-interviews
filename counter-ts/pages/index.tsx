...
function addObjects<T = any>(one: T, two: T) {
  return {
    one,
    two,
  }
...
export default function Home() {
  ...
  const [nextPageNumber, setNextPageNumber] = useState<string>(1)
  const [userInfos, setUserInfos] = useState([])
  const [randomUserDataJSON, setRandomUserDataJSON] = useState('')

  const item = addObjects<string, string>('one', 'two')
  console.log(item)
  const fetchUserData = (pageNumber: number = 1) => {
    // try {
    ...
    return axios
      .get<Response>(`https://randomuser.me/api?page=${pageNumber}`)
      .then((res) => {
        res.config.
  ...
  return (
    <>
      ...
      <main className="w-full max-w-containerXS">
