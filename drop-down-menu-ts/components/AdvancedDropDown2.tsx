...
export default function AdvancedDropDown2({}: Props) {
  ...
  return (
    <div className="relative flex h-[340px] w-[340px] flex-col items-center rounded-lg">
      <button
        ...
        className="flex w-full items-center justify-between rounded-lg border-4 border-transparent bg-blue-400 p-4 text-lg font-bold tracking-wide duration-200 active:border-white active:text-white"
      >
        ...
        <div className="absolute top-20 flex w-full flex-col items-start rounded-lg bg-blue-400 p-2">
          {emotes.map((item, index) => {
            return (
              <div
                onClick={() => {
                  setSelected((prev) => (prev = item['city']))
                  setIsOpen((prev) => !prev)
                }}
